...
PSFspacing=350:1:650; % array of wavelengths at which images are made and MTF is computed. Stay within 450 and 650. default is 450:5:650.
focallength=12; % focal length in mm. We assume f/1.2 beam at full aperture and so a 10 mm diameter lens. default is 12
lambdastride=1; % step size for wavelength. Leave this at 1
diagnosticplots=false; % set to true for verbose plotting
testrows=1000; % sets size of test pattern. default is 1000 and 1000. spatial scale is 1 micron
testcols=1000;

%%%% Frequently edited parameters begin here: pupil and focus choices 
%%%%%%%%%%%%%%%%%%%%%%%%%%%% begin selection section. INTEGERS ONLY %%%%%%%%%%%
bestfocuslambda=550; % best-focus wavelength in nm. Opsin peak is 500 **INTEGERS ONLY**
pupilouterradius=1; % pick 4 for full aperture, 1 for small central  **INTEGERS ONLY**

%%%%%%%%%%%%% end pupil selection section %%%%%%%%%%%%

PSFlabel=['PSF_' num2str(bestfocuslambda) '_' num2str(pupilouterradius)]; % no spaces, no slashes! Pupil type is appended automatically. Use color info. 
...
midx=floor(testcols/2);
midy=midx;
integral=0; 

for wavecounter=1:lambdastride:301 % this steps through wavelengths. Truncated range of wavelengths to conform to 
      ...
      % smaller by a factor of pupilscaling
        ...
        pupilrad(wavecounter)=1000*pupilouterradius*pupilscaling(wavecounter);
       
        
        for xcounter=1:testcols
            for ycounter=1:testrows
                pixrad=sqrt((xcounter-midx).^2+(ycounter-midy)^2);
                if (pixrad<=pupilrad(wavecounter))
        ...
        % trap on condition where it's a point smaller than a pixel
        if (integral==0)
            blurredimage(midx,midy,wavecounter)=1;
            integral=1;
        end
        
        % normalize the pupil
        blurredimage(:,:,wavecounter)=blurredimage(:,:,wavecounter)/integral;
    
        % scale to flux at this wavelength
        blurredimage(:,:,wavecounter)=blurredimage(:,:,wavecounter).*fullspectrum(wavecounter);
...
% compute FWHM bt finding place where intensity is 0.5
...
outvec1=[bestfocuslambda pupilouterradius FWHM];
