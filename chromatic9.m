% chromatic9.m, simulates chromatic aberration
% Released under GPL v2 license by Christopher Stubbs March 22 2015

% This program computes chromatically aberrations of an image, given
% intensity and reflectance information. It takes these inputs:
...
%  5) chromatic blur dependence on defocus
%  6) depth-dependent attentuation of sunlight.

% User can specify the pupil mask, and the two colors used in the test
% pattern.
...
% spanning 350 to 900 nm, and the chromatic blur as a function of
% wavelength. 
...
bestfocuslambda=495; % wavelength of sharpest focus, in nm. between 350-905

focallength=12; % focal length in mm. We assume f/1.2 beam at full aperture and 10 mm diameter lens
...
testcols=1000;
chirp_factor=25000; % this determines the bar pattern frequency change. Initial value was 100000
...
color1=grey;
color2=white; 
pupiltype='Annular Pupil';
savename='GryWht_annular_fullspectrum.mat';
figname='BlkWht_annular_fullspectrum.pdf';
...
% ** need to attenuate this for appropriate water depth if not shallow.
...
focusshift=-5.4676e-05*lambda.^2 + 0.0794*lambda -27.1047; % percent focus shift
...
blue_reflectance=fish(:,1)/2; % intensity fixed to get similar integrals in blue and yellow
...
barpattern=0.5+0.5*square(2*pi*(xpattern.^2.1)/chirp_factor); % makes 1's and 0's for bar pattern
...
% test to aggregate test images in a line
% dual=[testimage testimage]
% 
% imshow(dual)
% shg
...
focuscounter=0;
varyfocusstack2=zeros(26,testrows,testcols); % initialize array

for bestfocuslambda=350:10:600 % 
    
focuscounter=focuscounter+1 % increment this counter so it's 1 the first time through

%% compute offset from best-focus wavelength. 
% Find  the index that corresponds to best focus at this wavelength
bestindex=floor(bestfocuslambda)-349

% now compute magnitude of focus offset from that wavelength. 
...
% also collapse this to a simple 2-d array. The aqueeze() function 
% gets rid of pesky additional dimensions that are only 1 deep.

for showcounter=1:26
subplot(1,1,1)
showcounter
imshow(squeeze(varyfocusstack2(showcounter,:,:)))
shg
pause(0.1)
MTFmax(showcounter)=max(max(squeeze(varyfocusstack2(showcounter,100,780:950))));
MTFmin(showcounter)=min(min(squeeze(varyfocusstack2(showcounter,100,780:950))));
end

MTF=2*(MTFmax-MTFmin)./(MTFmax+MTFmin)

figure(34)
plot([350:10:600],MTF)
grid on
shg
%% snag MTF information
...
axes('FontName','Times New Roman') 
 ...
 %   title('focus at 400 nm')
    ...
    grid on
    subplot(3,1,2)
    plotline2=squeeze(varyfocusstack2(2,100,:));
    plot(plotline2,'k')
    axis([1 950 0 1])
    grid on
  %  title('focus at 450 nm')
    subplot(3,1,3)
    plotline3=squeeze(varyfocusstack2(3,100,:));
    plot(plotline3,'r')
    axis([1 950 0 1])
   % title('focus at 500')
    grid on
...
imagef1=squeeze(varyfocusstack2(1,:,1:950)); % focus 450 nm
imagef2=squeeze(varyfocusstack2(2,:,1:950)); % focus 500 nm
imagef3=squeeze(varyfocusstack2(3,:,1:950)); % focus 550 nm

linecutf1=squeeze(varyfocusstack2(1,100,1:950));
linecutf2=squeeze(varyfocusstack2(2,100,1:950));
linecutf3=squeeze(varyfocusstack2(3,100,1:950));
...
imshow(imagef1,'InitialMagnification','fit')
...
grid on
plot([350:10:600],MTF,'k')
xlabel('wavelength, nm','FontName','Times','FontSize',16)
