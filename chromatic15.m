% chromatic15.m, simulates chromatic aberration for Cephalopods
% Released under GPL v2 license by Christopher Stubbs 

% version 15, March 27 2015
...
% this next parameter determines the bar pattern frequency change.  Default is 10000. smaller numbers give higher freq
% this is a balance between getting a wide dynamic range in frequency but numbers below 10,000 have aliasing for annular pupil. 
% 8000 is smallest sensible value for full pupil.
...
MTFcut=1.9; % threshold for fractional MTF computation
...
color1=black;
color2=white; % be sure to change file name below as appropriate

MTFlabel='BlkWhtAnnularPSF'; % no spaces, no slashes! Suggest pupil (full/small/annular) and the two colors. 
savename=[MTFlabel '.mat'];
...
% green_reflectance=0.001+exp(-((lambda-lambda(center2)).^2)/(2*sigma^2));
...
for bestfocuslambda=min(MTFspacing):(MTFspacing(end)-MTFspacing(end-1)):max(MTFspacing)
bestfocuslambda
...
% response. 
  ...
  for counter=101:301 % this steps through wavelengths, 450 to 650 nm. Truncated range of wavelengths to conform to 
                     % Jagger and Sands range of focus shifts.  use
                      % counter=101:301 for 450 to 650. For PSF images use
                      % 1:5:301
    ...
    status=['at wavelength ' num2str(lambda(counter)) ' for focus setting ' num2str(bestfocuslambda)] % spit out loop counter for diagnostic purposes
...
subplot(2,1,2)
plot(1:testcols,squeeze(varyfocusstack2(showcounter,100,:)),'r')
ylim([0 1.1])
shg
pause(0.3)

%%% compute image quality factor MTF. 


% normalize to excursions above/below the mean flux level
relativeFlux=squeeze(varyfocusstack2(showcounter,100,:));
relativeFlux=relativeFlux/mean(relativeFlux);
% 
% plot(relativeFlux,'k.')
% shg

% there are 500 potential pixels that might be above the mean
MTF2(showcounter)=sum(relativeFlux > MTFcut)/500;


end

% This is a vector with dimension equal to number of focus settings 
MTF=MTF2; % use alternate unaliased version 
...
axis([-0.35 0.2 -0.1 1.1]);
text(-0.32,1.0,MTFlabel,'FontName','HELVETICA','FontSize',20) % for some reason this does not appear in saved JPEG!
hold off
shg
...
axis([300 900 0 1])
set(gca,'xtick',[])
%add second axis label here for accom
